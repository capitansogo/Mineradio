...
- Страница исполнителя для Яндекса: инфо + популярные треки (через `brief-info`).
- Тексты песен через LRCLIB: универсальный фолбэк на открытые **синхронные** LRC, когда у провайдера (Яндекс/NetEase/QQ) текста нет. Резко улучшает покрытие, в т.ч. для андеграунда.

## v1.1.3 (RU-форк)

Яндекс Музыка:
- Синхронные тексты песен (LRC) + fallback на простой текст с распределением по длительности.
- «Моя волна» — персональное радио с непрерывной дозагрузкой (rotor), без зацикливания на первый трек.
- Главная под Яндекс: карточки «Плейлист дня» и «Дежавю» из ленты; пустые NetEase-карточки скрываются.
- Лайки (♥ → «Мне нравится»), синхронизация состояния.
- «Мне нравится»: реальный счётчик треков, обложка-мозаика и воспроизведение (резолв полных треков).
- Кнопка качества звука скрывается на яндекс-треках (только mp3).

Производительность и UI:
- Оконный рендер очереди (плавный скролл на 1000+ треках вместо рендера всего разом).
- Прокси обложек host-aware (корректные заголовки для Яндекс/QQ).
- Множество правок русификации: метки качества/режима, тосты, подписи карточек, шапка плейлиста, строка поиска, инсталлятор.
- Скруглённые уголки детали плейлиста; убрана подсказка про NetEase/QQ при входе только в Яндекс.

## v1.1.2 (RU-форк)

- Исправлена упаковка: `yandex.js` добавлен в сборку (без него установленная версия не запускалась).
